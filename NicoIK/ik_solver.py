...
import pandas as pd
from camera_perspective import rotate_around_z
...
def target():
    target_position = [0.30 + (0.25 * random.rand()), 0.0 + (-0.25 * random.rand()),
...
def get_joints_limits(robot_id, num_joints, arg_dict):
    ...
    return [joints_limits_l, joints_limits_u], joints_ranges, joints_rest_poses, joint_names, link_names, joint_indices
...
def main():
    ...
    parser.add_argument("-p", "--position", nargs=3, type=float,
                        help="Target position for the robot end effector as a list of three floats.")
    ...
    parser.add_argument("-i", "--initial", action="store_true",
                        help="If set, reset the robot to the initial position after each postion")
    parser.add_argument("-t", "--trajectory", action="store_true",
                        help="Store coordinates from simulation into text file")
    ...
    parser.add_argument("-f", "--forward", action="store_true", help="If set, direct kinematics is computed")
    ...
    # Load the URDF robot create a scene

    if arg_dict["left"]:
        robot_id = p.loadURDF("./nico_upper_rh6d_l.urdf", [0, 0, 0])
    else:
        robot_id = p.loadURDF("./nico_upper_rh6d_r.urdf", [-0.05, 0, 0])
    # Create table mesh
    p.createMultiBody(baseVisualShapeIndex=p.createVisualShape(shapeType=p.GEOM_BOX, halfExtents=[.3, .45, 0.02],
                      ...
                      baseMass=0, basePosition=[0.27, 0, 0.02])
    # Create tablet mesh
    ...
    # array_of_3d_points.append(result[0])

    for i in range(len(array_of_3d_points) - 1):
        start_point = array_of_3d_points[i]
        end_point = array_of_3d_points[i + 1]
        p.addUserDebugLine(start_point, end_point, lineColorRGB=[1, 0, 0], lineWidth=2)

    # (1, b'head_z_rjoint', 4, -1, -1, 0, 0.0, 0.0, -1.5708, 1.5708, 30.0, 1.0, b'neck', (0.0, 0.0, 0.0), (-0.025, 0.004734, 0.179648), (0.0, 0.0, 0.0, 1.0), 0)

    print("=======")

    # Custom intital position
    # posledne dve hodnoty 0 -> -90

    joints_rest_poses = deg2rad([-15, 68, 2.8, 100, 0.0, 11.0, 0.0, 0, 0, 0, 0])
    ...
    for i in range(30):
        ...
        # precision sequence 4
        import json, time
        ## 2D coordinates file
        coordinates = open(f"Trajectories.3.vd", "a")
        if arg_dict["forward"]:
            for repeat in range(1):
                ## load motor dataset
                with open('dataset.json', 'r') as file:
                    dataset = json.load(file)
                    ## for each sequence for each grasp type for each perspective and for each position
                    for sequence in range(10):
                        for grasp_index, grasp_type in enumerate(['power', 'precision', 'side']):
                            for perspective_index, perspective in enumerate([0, 90, 180, 270]):
                                for pos_index, pos in enumerate(dataset[grasp_type][f'sequence {sequence}'].values()):
                                    ## rescale fingers from interval <0,180> to angles
                                    other_fingers = calculate_nico_joint(pos[9], [0, 180], FINGER_RANGE)
                                    index_finger = calculate_nico_joint(pos[8], [0, 180], FINGER_RANGE)
                                    thumb_lift = calculate_nico_joint(pos[7], [0, 180], THUMB_RANGE)
                                    thumb_close = calculate_nico_joint(pos[6], [0, 180], THUMB_RANGE)
                                    ## covert degrees to radians
                                    joints_rest_poses = deg2rad(
                                        [pos[0], pos[1], pos[2], pos[3], pos[4] * 0.4, pos[5] - 20
                                            , other_fingers, other_fingers, index_finger,
                                         thumb_lift, thumb_close])
                                    ## set joints in pybullet simulator
                                    for i in range(len(joint_indices)):
                                        p.resetJointState(robot_id, joint_indices[i], joints_rest_poses[i])
                                    line = f'{sequence + 1};{grasp_index + 1};{perspective_index};{pos_index + 1};'
                                    ## acquire visual data from pybullet simulator
                                    for i in range(3, 14):

                                        try:
                                            (x, y, z), (a, b, c, d), _, _, _, _ = p.getLinkState(robot_id, i)
                                            line += rotate_around_z([x, y, z], perspective) + ';'
