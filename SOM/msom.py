...
class MSOM:
    def __init__(self, input_dimensions, output_dimensions, alpha=0.1, beta=0.1, gamma=0.1, sigma=1.0):
        self.input_dimensions = input_dimensions
        self.output_dimensions = output_dimensions
        self.alpha = alpha  # learning_rate
        self.beta = beta  # context_rate
        self.gamma = gamma  # context_rate
        self.sigma = sigma
        ...
        for epoch in range(epochs):
            ...
            for seq in data:
                for t, x in enumerate(seq):
                    ...
                    self.update_weights(x, best_matching_unit, epoch, num_samples)
            print(str(self.qError)+',')
    ...
    def update_weights(self, x, best_matching_unit, epoch, num_samples):
        ...
        for i in range(self.output_dimensions[0]):
            for j in range(self.output_dimensions[1]):
                ...
                self.context_weights[i, j] += self.gamma * gamma * (x - self.context_weights[i, j])
    ...
    def calculate_distance(self, x, i, j):
        ...
        return (1 - self.alpha) * np.linalg.norm(x - input_weight) + self.alpha * np.linalg.norm(context_weight - input_weight)
